...
def inception_block(net, scale=1.0, activation_fn=tf.nn.relu, scope=None, reuse=None):
...
def reduction_block(net, atrous=False):
...
def inception_resnet_slim_base(inputs,
  ...
  with tf.variable_scope(scope, 'InceptionResnetV2', [inputs]):
    with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d],
      ...
      # 8
      net = slim.max_pool2d(net, 2, stride=2, padding=padding,
                            scope='maxpool1')
      ...
      net = slim.repeat(net, 10, inception_block, scale=0.2,
                        activation_fn=activation_fn)
      if add_and_check_final('block2', net): return net, end_points

      # 4
      net = reduction_block(net)
